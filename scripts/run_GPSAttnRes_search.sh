...
LOG_DIR="logs/gps_attnres_search"
mkdir -p "$LOG_DIR"
...
REPORT_EVERY_N_EPOCHS="${REPORT_EVERY_N_EPOCHS:-5}"
CPUS_PER_TRIAL="${CPUS_PER_TRIAL:-4}"
GPUS_PER_TRIAL="${GPUS_PER_TRIAL:-1}"
NUM_WORKERS="${NUM_WORKERS:-2}"
OUTPUT_DIR="${OUTPUT_DIR:-./results/gps_attnres}"
STORAGE_PATH="${STORAGE_PATH:-/tmp/ray_results/gps_attnres}"
TASKS=(charge energy)
MODELS="${MODELS:-gps_attnres_gp}"
...
echo "============================================================" | tee "${run_log}"
echo "[$(date '+%F %T')] Running GPSAttnRes hyperparameter search" | tee -a "${run_log}"
...
echo "Charge search: batch=256, layers in {24,28,32,36}, hidden in {128,160,192,224}" | tee -a "${run_log}"
echo "Energy search: batch=512, layers in {12,16,20,24,28}, hidden in {96,128,160,192}" | tee -a "${run_log}"
echo "Results: live_search_<task>.csv updates during the run; search_<task>.csv is written after the task finishes" | tee -a "${run_log}"
echo "============================================================" | tee -a "${run_log}"
...
echo "[$(date '+%F %T')] GPSAttnRes hyperparameter search completed." | tee -a "${run_log}"
