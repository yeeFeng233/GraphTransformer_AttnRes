...
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/logs/gps_attnres_energy_search}"
mkdir -p "$LOG_DIR"
...
REPORT_EVERY_N_EPOCHS="${REPORT_EVERY_N_EPOCHS:-10}"
CPUS_PER_TRIAL="${CPUS_PER_TRIAL:-4}"
GPUS_PER_TRIAL="${GPUS_PER_TRIAL:-1}"
NUM_WORKERS="${NUM_WORKERS:-8}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/results/gps_attnres_energy}"
STORAGE_PATH="${STORAGE_PATH:-${PROJECT_DIR}/ray_results/gps_attnres_energy}"
RAY_TEMP_TARGET="${RAY_TEMP_TARGET:-${PROJECT_DIR}/ray_tmp/gps_attnres_energy}"
# Ray creates UNIX sockets under the temp dir; this visible path must stay short.
RAY_TEMP_DIR="${RAY_TEMP_DIR:-/tmp/echo_ray_energy_${USER:-user}}"
TASKS=(energy)
...
echo "Energy search stage-4: long training for unstable energy curves; batch=512, layers in {26,28,30}, hidden in {160,192,224}, stride in {2,4}" | tee -a "${run_log}"
