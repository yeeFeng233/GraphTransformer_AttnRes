...
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/logs/gps_attnres_energy_search_part2}"
mkdir -p "$LOG_DIR"
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/results/gps_attnres_energy_part2}"
STORAGE_PATH="${STORAGE_PATH:-${PROJECT_DIR}/ray_results/gps_attnres_energy_part2}"
RAY_TEMP_TARGET="${RAY_TEMP_TARGET:-${PROJECT_DIR}/ray_tmp/gps_attnres_energy_part2}"
# Ray creates UNIX sockets under the temp dir; this visible path must stay short.
RAY_TEMP_DIR="${RAY_TEMP_DIR:-/tmp/echo_ray_energy_part2_${USER:-user}}"
TASKS=(energy)
TRAINING_LOG_DIR="${TRAINING_LOG_DIR:-${LOG_DIR}/training_logs}"
MODELS="${MODELS:-gps_attnres_gp_part2}"
...
echo "Energy search part2: patience-driven long training; batch=512; see search-space/energy/gps_attnres_gp_part2.yaml" | tee -a "${run_log}"
