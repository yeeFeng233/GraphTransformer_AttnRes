...
# Generic ECHO-Synth GPS+AttnRes search launcher.
...
MODEL_NAME="${MODEL_NAME:-gps_attnres_gp}"
...
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/logs/gps_attnres_${TASK_NAME}_search}"
mkdir -p "${LOG_DIR}"
...
if [ "${DETACH}" -eq 1 ]; then
  echo "Starting GPS+AttnRes ${TASK_NAME} search in detached mode..."
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/results/gps_attnres_${TASK_NAME}}"
STORAGE_PATH="${STORAGE_PATH:-${PROJECT_DIR}/ray_results/gps_attnres_${TASK_NAME}}"
RAY_TEMP_TARGET="${RAY_TEMP_TARGET:-${PROJECT_DIR}/ray_tmp/gps_attnres_${TASK_NAME}}"
# Ray creates UNIX sockets under the visible temp path; keep it short.
RAY_TEMP_DIR="${RAY_TEMP_DIR:-/tmp/echo_ray_gpsar_${TASK_NAME}_${USER:-user}}"
...
echo "============================================================" | tee "${run_log}"
echo "[$(date '+%F %T')] Running GPS+AttnRes ${TASK_NAME} hyperparameter search" | tee -a "${run_log}"
...
echo "[$(date '+%F %T')] GPS+AttnRes ${TASK_NAME} search completed." | tee -a "${run_log}"
